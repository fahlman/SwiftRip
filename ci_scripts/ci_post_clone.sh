...
if [[ -n "${CI_WORKSPACE:-}" && ! -d "$REPOSITORY_DIR/SwiftRip-Tools" && -d "$CI_WORKSPACE/repository" ]]; then
    REPOSITORY_DIR="$CI_WORKSPACE/repository"
fi

FETCH_SCRIPT="$REPOSITORY_DIR/SwiftRip-Tools/Scripts/fetch-swiftrip-tools.zsh"
...
for arch in arm64 x86_64; do
    echo ""
    echo "Fetching SwiftRip-Tools artifacts for $arch..."
